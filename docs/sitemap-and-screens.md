...
| `/profiles/{id}/qr` | QR表示 | 共有イベントの内容確認とQR再表示を行うPWA内画面。 |
...
ユーザー自身は、共有イベントがまだサーバーへアップロードされていない場合でも、PWA内に保存されたイベント内容を確認できる。

表示項目:

- 共有プロフィール名
- QRコード
  - QRコードの中心には　共有プロフィール（ないばいはアカウントプロフィール）を表示する
    - どちらもない場合は画像は表示せずQRのみとする
- 共有イベントのメッセージ
- 共有イベントの写真
- 共有イベントの位置情報取得状態
- アップロード状態
...
- 共有イベントが未アップロードでも、ユーザー自身にはPWA内の端末保存データを使ってイベント内容を表示する。
...
ユーザーはPWA内のイベント一覧やQR表示画面から未アップロードのイベント内容を確認できるが、ゲスト側では共有イベント本体がサーバーに存在しないため内容を表示しない。
...
| `share_events` | ダッシュボード、QR表示、共有イベント公開ページ | その場で作った共有イベント。PWA内では未アップロード状態でも内容確認に使う。 |
| `share_event_photos` | ダッシュボード、QR表示、共有イベント公開ページ | 共有イベントに添付した写真。PWA内では未アップロード状態でも内容確認に使う。 |
...
| `guest_messages` | 共有イベント公開ページ | ゲストから所有者へのひとことメッセージ。ユーザー側PWAのイベント内容確認画面には表示しない。 |
