...
共有プロフィールは、ダッシュボードの共有プロフィール一覧から編集画面へ進み、参照と編集を同じ画面内で扱う。
...
| `/profiles/new` | 共有プロフィール作成 | 相手に見せる共有プロフィールを作成する画面。 |
| `/profiles/{id}` | 共有プロフィール参照・編集 | 互換用。アクセス時は `/profiles/{id}/edit` へ遷移する。 |
| `/profiles/{id}/edit` | 共有プロフィール参照・編集 | 既存の共有プロフィールを確認・編集する画面。 |
| `/share-events/{id}/qr` | QR表示 | 作成済み共有イベントのQRを表示・再表示するPWA内画面。イベント詳細は表示しない。 |
| `/share-events/qr` | QR表示 | 未同期ドラフト用の予約済み共有イベントQRを表示するPWA内画面。 |
...
- 共有ブロック
  ...
  - 表示想定人数
    ...
    - デフォルト値は `3` とする
    - 1以上の整数を入力する
    - 表示想定人数は、このイベントを見せる予定の実人数として扱う
    - 将来、この値が設定されている場合は、イベントごとのユニークゲストアクセス数が「表示想定人数 × 3」以上になった時点でアクセスを制限する
    - ユニークゲストアクセス数はページ表示回数ではなく、イベント別Cookieのゲスト識別IDを基準に数える
    - 1人がPCやスマートフォンなど最大3端末・ブラウザからアクセスする想定で、表示想定人数に3を掛ける
    - アクセス制限機能と制限時画面は今回は実装せず、将来対応とする
  - 位置情報をつける
    - ON/OFFのスライダートグルで表示し、デフォルトはONとする
    - ブラウザで位置情報を利用でき、サイト単位で拒否済みでない場合に操作可能にする
    - 位置情報が許可済みの場合は、トグルのすぐ下に「位置情報をつけると地図を添付できます」と表示する
    - 位置情報が拒否済み、またはブラウザで利用できない場合はトグルを無効化する
    - トグルが無効な場合は、その下に「位置情報を許可すると使えます」と表示する
    - 位置情報が許可済みの場合は「位置情報を許可すると使えます」を表示しない
    - ONの場合だけイベントの初回作成時に位置情報を取得・保存し、OFFの場合は取得しない。イベント一覧からQRを再表示するときは位置情報を取得・更新しない
  - 共有QR作成ボタン

- 共有プロフィール一覧ブロック
  - 編集画面への導線のみ表示する
  - 共有プロフィール単体のQR表示導線は持たせない
...
目的:
相手に見せるプロフィールを用途別に作り、既存プロフィールの内容確認と編集を同じ画面で扱う。仕事用、趣味用、イベント用などを分けられるようにする。
...
- SNS/外部リンク一覧
  - 登録済みの連絡先だけを表示する
  - 各行にサービスアイコン、サービス名、入力値、削除ボタンを表示する
- 連絡先追加ボタン
  - 押すと追加モーダルまたはボトムシートを表示する
  - 追加候補はSNS/連絡先種類マスタから取得する
  - アイコンを縦横グリッドで表示する
  - 未登録アイコンを押すと、そのサービスの入力欄をプロフィール編集画面へ追加する
  - 登録済みアイコンを押した場合は、該当入力欄へスクロールする
  - 削除は入力欄側の削除ボタンで行う
- 最近の共有イベント
- イベント写真
- 保存ボタン
...
- 既存プロフィールの参照と編集は別ページに分けず、同じ編集画面内で扱う。
- 共有プロフィール画面からQR表示へ進む導線は持たせない。QRは共有イベント作成後または共有イベント一覧から再表示する。

### QR表示

目的:
共有イベント作成直後またはイベント一覧から開き、相手に見せるQRを大きく表示する。通信が不安定でも、相手が画面を写真に撮って後からアクセスできるようにする。
この画面はPWA内の画面として扱う。
...
- 共有イベントQRを相手に見せる
- ダッシュボードの共有イベント一覧からQRを再表示する
- URLを開く

注意:

- QR再表示時は位置情報の許可を求めず、位置情報を取得・更新しない。
...
- プロフィールブロック
  ...
  - 自己紹介
  - プイロフィールSNSブロック
    - 登録済みのSNS/外部リンクをアイコングリッドで表示する
    - アイコンをタップするとアクションメニューを表示する
      - サービス名
      - コピー
      - リンク先へ移動
...
- 有効期限
- 表示想定人数（数値入力、デフォルト `3`）
- 位置情報をつけるトグル（位置情報の許可状態に応じて有効・無効と補足文を切り替える）
- 現在の通信状態
- 予約済みトークン状態
- QRプレビュー
- QR作成ボタン
...
| `sns_types` | 共有プロフィール作成・参照・編集、共有イベント公開ページ | SNS/連絡先種類マスタ。サービス名、アイコン、カテゴリ、入力種別、URL/コピー用テンプレートを管理する。 |
| `profile_sns` | 共有プロフィール作成・参照・編集、共有イベント公開ページ | 共有プロフィールに登録されたSNS/外部リンク/連絡先。 |
| `share_events` | ダッシュボード、QR表示、共有イベント公開ページ | その場で作った共有イベント。QR表示では共有イベントURL生成と位置情報保存の対象として使い、イベント詳細表示はゲスト公開ページで行う。 |
